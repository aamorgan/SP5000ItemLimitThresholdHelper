...
namespace TSOTestApp
{
    public partial class TSOTester : Form
    {
        ...
        public TSOTester()
        {
            ...
            //        "<application>"+
            //        "    <id>profile-ws</id>" +
            //        "    <name>Profile RESTful Web Service</name>" +
            //        "    <version>6.1.23.102</version>" +
            //        "    <state>0</state>" +
            //        "    <since>2020-05-11T13:08:37.332-04:00</since>" +
            //        "    <authorization>" +
            //        "        <required>true</required>" +
            //        "        <authenticated>true</authenticated>" +
            //        "        <username>MORGAN.ANDREW.ANTHONY.1558203346</username>" +
            //        "        <roles>Public</roles>" +
            //        "    </authorization>" +
            //        "</application>";
            //XmlDocument doc = new XmlDocument();
            //doc.LoadXml(what);
            //var dis = doc.InnerXml;
            InitializeComponent();
        }
        #endregion
        ...
        private HttpWebResponse ExecuteWebRequest(string url)
        {
            if (!string.IsNullOrEmpty(url)) // This counts on Username now carrying the certificate name!!
            {
                ...
                if (null != webRequest)
                {
                    ...
                    ServicePointManager.SecurityProtocol = (cbTLS12.Checked) ? Tls12 : _securityProtocolType;
        ...
        private X509Certificate2Collection GetCertCollection(bool UseCAC)
        {
            ...
            else
            {
                ...
                X509Store store = new X509Store(cbStore.Text, rbLocal.Checked ? StoreLocation.LocalMachine : StoreLocation.CurrentUser);
        ...
        private void btnCheckCerts_Click(object sender, EventArgs e)
        {
            ...
            SetCertSet();
            btnGetData.Enabled = collection.Count != 0;
        ...
        {
            //txtCertCriteria1.Text = (cbStore.SelectedIndex == 1) ? "CN=EUSTNB4351911, OU=Computers, OU=G-6, OU=TRADOC HQ, OU=TRADOC, OU=Eustis, OU=Installations, DC=nae, DC=ds, DC=army, DC=mil" 
            //    : "CN=portal2.tradoc.army.mil, OU=USA, OU=PKI, OU=DoD, O=U.S. Government, C=US";
        }
    }
}
